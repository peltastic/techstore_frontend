...
import { addCart, checkCart, increaseCart, decreaseCart } from "../../../../api/requests/cart";
...
function Description({}: Props) {
  ...
  return (
    <>
      <Nav />
      <div className="text-white mt-[7rem] border-white border w-[90%] h-[80vh] flex flex-wrap">
        ...
        <div className="w-[50%] relative">
          ...
          <p className="my-8 text-xl">N{totalPrice? totalPrice: data?.price}</p>
          {cartCount < 1 ? (
            <button
          ...
          {cartCount  ? (
            <div className="w-[60%] flex items-center text-3xl justify-between">
              <button  disabled={cartCount===0} onClick={decreaseCartHandler} className="bg-[#B3541E] rounded-full h-16 w-16">
                <AiOutlineMinus className="m-auto text-5xl" />
              </button>
              <p>{cartCount}</p>
              <button onClick={increaseCartHandler} className="bg-[#B3541E] rounded-full h-16 w-16">
